...
![image-20230629133908094](https://p.ipic.vip/9bf5pp.png)
...
With this formulation, there are definitely holes in the **virtual memory space**.
...
* RPL: Requestor’s Privilege Level (RPL of CS Þ Current Privilege Level)
...
**64-bit ("long") Mode**: In the 64-bit mode of x86 processors, most segments (SS, CS, DS, ES) are set up with zero base and no length limits, effectively covering the entire 64-bit address space. However, the FS and GS segments still retain their functionality and can be used for Thread Local Storage (TLS).
...
A page table entry is a pointer to next-level page table or to actual page. The Intel x86 archtecture PTE for the last level is like:
...
| PS      | Page Size: PS=1Þ4MB page (directory only).Bottom 22 bits of virtual address serve as offset |

For the first few levels of table, all the 32 bits are used to point to a next-level PTE.
...
>  **About Segment**
>
> 1. **Text Segment (Code Segment)** (CS): This contains the executable code of the program.
> 2. **Data Segment** (DS): This segment usually contains the global and static variables that are initialized by the programmer.
...
> 4. **Stack Segment** (SS): This contains the program's runtime stack. Local variables and function call information typically reside here.
